module game {
    ...
    export class TextUtils {
        ...
        public static showTextTip(tip: string, color: number = 0xFFFFFF): void {
            var max_W: number = StageUtils.getInstance().getStageWidth();
            var max_H: number = StageUtils.getInstance().getStageHeight();
            var textfield: egret.TextField = game.TextUtils.createTextfield(30);
            textfield.text = tip;
            textfield.textColor = color;
            ...
            StageUtils.getInstance().getStage().addChild(textfield);
